...
class ValueImportProcessorSystemExtensionPolicyAllowedTypes: ValueImportProcessor {
    ...
    override func addValue(toCurrentValue: [Any]?, cellView: PayloadCellView, completionHandler: @escaping (_ value: Any?) -> Void) throws {
        ...
        let infoDictionary = systemExtensionBundle.infoDictionary;
        var extensionTypesFound = Set<String>()
        
        if infoDictionary?["NetworkExtension"] != nil {
            extensionTypesFound.insert("NetworkExtension")
        }
        
        if infoDictionary?["EndpointExtension"] != nil {
            extensionTypesFound.insert("EndpointExtension")
        }
        
        if infoDictionary?["DriverExtension"] != nil {
            extensionTypesFound.insert("DriverExtension")
        }
